...
class HazAngular implements IEntidadCa{
    private  $db;
    ...
    public function insertar($arrayCaracterizacion) {
		...
		$k=implode(",",$k);
	    $v=implode(",",$v);
		$sql="INSERT INTO tsreportehazangular ($k) values ($v)";
        try{
    ...
    public function buscar($clNumReporte) { 
        $sentencia = "SELECT * FROM tsreportehazangular WHERE clNumReporte = :clNumReporte ORDER BY id DESC LIMIT 1";
        try{
    ...
    public function buscarID($clNumReporte) { 
        $sentencia = "SELECT id FROM tsreportehazangular WHERE clNumReporte = :clNumReporte ORDER BY id DESC LIMIT 1";
        try{
            $stm = $this->db->prepare ($sentencia);
            $stm->bindValue(':clNumReporte', $clNumReporte);
            $stm->execute();
    ...
    public function buscarReporte($clNumReporte) { 
        $sentencia = "SELECT * from tsreportehazangular rc join tsimghazangular using(id) WHERE rc.clNumReporte = :clNumReporte";
        try{
            $stm = $this->db->prepare ($sentencia);
            $stm->bindValue(':clNumReporte', $clNumReporte);
            $stm->execute();
    ...
    public function buscarClave($clave) { 
        $sentencia = "SELECT * from tsreportehazangular c join tsimghazangular using(id) WHERE c.id = :clave";
        try{
            $stm = $this->db->prepare ($sentencia);
    ...
    public function cliente($cliente) { 
        $sentencia = "SELECT clNumReporte from tsreportehazangular WHERE cacliente_clCliente = :cliente ORDER BY id DESC LIMIT 1";
        try{
            $stm = $this->db->prepare ($sentencia);
    ...
    public function modificar($clNumReporte, $arrayCaracterizacion) {
        $sql= "UPDATE tsreportehazangular SET ";
